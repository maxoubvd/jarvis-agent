...
- `~/.jarvis/config.json` (global) + `<workspace>/jarvis/jarvis-config.json` (workspace override, deep-merged) — model-centric schema: `models.items: ModelItem[]` (`provider`, `apiKey`/`apiBase`, `roles`, `contextLength`), `rules` (with optional `scope` glob), `agents`, `workflows`, `prompts`, `webSearch` (`{provider: 'duckduckgo', defaultSites}`), `optimization` (hitl/chat/verbosity/auto-open/autocomplete settings).
