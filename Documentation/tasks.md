...
- [x] Ajouter un thème VS Code intégré et les variables CSS natives
- [x] Intégrer le build Vite pour générer `dist/webview`
- [x] Créer `Documentation/init frontend.md`
...
- [x] Ajouter des providers de base :
  - [x] `Ollama`
  - [x] `OpenRouter`
- [x] Ajouter le support d'un provider local supplémentaire si nécessaire
- [x] Ajouter la gestion de configuration dynamique des modèles via `jarvis-config.json`
...
- [x] Créer un document `tasks.md` (en cours)

### 1.5. Tests de base

- [x] Ajouter la configuration de tests unitaires (`vitest`, `@testing-library/svelte`, etc.)
- [x] Créer des tests unitaires pour le backend et le frontend
- [x] Vérifier le build et exécuter un premier test
...
- [x] Implémenter l'outil terminal MCP complet (`terminal.ts` — `child_process.spawn`)
- [x] Capturer stdout/stderr en temps réel (callback `onData`)
- [x] Afficher les résultats de commandes dans l'UI ou le chat (`terminalOutput` → webview)
- [x] Ajouter la validation de sécurité avant exécution (HITL)

### 2.2. Lecture et modification de fichiers

- [x] Implémenter `write_file` (réalisé en Phase 1 — `fileSystem.ts`)
- [x] Implémenter `edit_file` (`editFileTool` — remplacement par plage de lignes)
- [x] Valider les chemins pour empêcher l'accès hors workspace (`SandboxManager.canAccess`)
- [x] Ajouter la gestion des erreurs en cas de permission manquante
- [x] Bonus : `listDirectoryTool`, `deleteFileTool`

### 2.3. HITL (Human-in-the-Loop)

- [x] Créer le système HITL Manager (`services/hitl.ts`)
- [x] Implémenter les modes : `strict`, `modéré`, `libre`
- [x] Ajouter des confirmations utilisateur pour les actions sensibles (modales VS Code)
- [ ] Charger la configuration HITL depuis `jarvis-config.json` (mode par défaut : moderate)

### 2.4. Sandbox et secrets

- [x] Créer `SandboxManager` (`core/utils/sandbox.ts`)
- [x] Ajouter la validation de path (+ génération auto de `.jarvisignore`)
- [x] Implémenter `SecretScrubber` (`core/utils/scrubber.ts`)
- [x] Ajouter les patterns de secrets par défaut (11 patterns)
- [x] Appliquer le scrubbing sur les lectures de fichiers (`@read`)
- [ ] Protection des symlinks sortants (à compléter)

### 2.5. Checkpoints Git

- [x] Créer `CheckpointManager` (`services/checkpoint.ts`)
- [x] Implémenter la création de checkpoints Git (`git stash` nommé)
- [x] Implémenter rollback via `git stash`
- [x] Ajouter une UI / commandes pour rollback (`CheckpointPanel.svelte` + `jarvis.listCheckpoints`)
- [x] Ajouter notifications de checkpoint
...
- [x] Créer la structure des fichiers de stats dans `.vscode/jarvis-stats.json`
- [x] Implémenter `AnalyticsCollector` (`services/analytics.ts`)
- [x] Suivre les tokens par modèle
- [x] Suivre les actions et les résultats
- [x] Ajouter export JSON (`jarvis.exportAnalytics`)
- [ ] Ajouter un dashboard UI Svelte avec graphiques (données collectées, UI à faire)
- [ ] Ajouter export CSV + réinitialisation des stats
